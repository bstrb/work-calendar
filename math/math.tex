...
Setting \(z=t\) and using \(\Omega^2=s^2+1/\xi^2\), we obtain
\[
I_{\mathrm{dyn}}(t,s)
\propto
...
\]
so that \(\sin^2(\pi x)=(\pi x)^2\operatorname{sinc}^2(x)\), this becomes
\[
I_{\mathrm{dyn}}(t,s)
\propto
\pi^2\left(\frac{t}{\xi}\right)^2
...
A simple way to obtain a nontrivial absorptive modification of the rocking curves is to allow the off-diagonal two-beam coupling to become complex:
\[
\gamma=
...
\]
Then, by direct analogy with the non-absorbing two-beam result, or equivalently by the replacement \(1/\xi \mapsto \gamma\),
\[
...
Repeating the integrating-factor calculation from Sec.~\ref{sec:kinematical}, now with \(1/\xi\) replaced by \(\gamma\), gives
\[
...
\subsection{Common complex coupling and final forms}

The absorbing kinematical and dynamical formulas are therefore both written in terms of the same complex coupling
\[
\gamma=\frac{1}{\xi}+i\frac{1}{\xi'}.
\]
In the kinematical limit, \(\gamma\) multiplies the source term and therefore rescales the \(\operatorname{sinc}^2(st)\) profile. In the dynamical case it also enters
\[
\Omega_{\mathrm{abs}}=\sqrt{s^2+\gamma^2},
\]
so it changes the rocking-curve shape itself. The final forms are
\[
I_{\mathrm{kin,abs}}(t,s)
\propto
